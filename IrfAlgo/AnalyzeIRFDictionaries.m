function [ ] = AnalyzeIRFDictionaries( irfData )
clc
...
for conditionsIdx = 1 : numel(maskingConditions)
    ...
    AnalyzeDifferencesOfSameSubjSameDiacriticType(irfData, conditions);

    %ComputeBestAccurracyConfigurationForEachSubjectRun(irfData, conditions);
    
    %ComputeMeanAccurracyConfiguration(irfData, conditions);
end
...
function [] = AnalyzeDifferencesOfSameSubjSameDiacriticType(irfData, conditions)
    
    disp(' ');
    answer = input(['Press "y" to display the correlation grades for irfs of specific subjects with a specific diacritic type ' mat2str(conditions) ' -> '], 's');
    if (strcmp(answer,'y') == 0) 
        return;
    end
    
    % get all the keys of configurations
    configurations = irfData.keys();
